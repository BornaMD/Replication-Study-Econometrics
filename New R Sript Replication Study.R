...
library(AER)
reg1_female.2SLS <- ivreg(agriculture_top~ln_eucldist_bu+married+ageyr
                  +factor(school_grade)+head+child_adult_7
                  +size+married_head+rain_sd
                  +district_population_pc+factor(ward)
                  +time+factor(id)| female + married+ageyr
                  +factor(school_grade)+head+child_adult_7
                  +size+married_head+rain_sd
                  +district_population_pc+factor(ward)
                  +time+factor(id))
summary(reg1_female.2SLS)

reg1_birth.2SLS <- ivreg(agriculture_top~ln_eucldist_bu+married+ageyr
                          +factor(school_grade)+head+child_adult_7
                          +size+married_head+rain_sd
                          +district_population_pc+factor(ward)
                          +time+factor(id)| female married+ageyr
